...
@implementation PDBrowserImageView
{
    __weak PDWaitingView *_waitingView;
    BOOL _didCheckSize;
...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        if (!_scroll) {
            ...
            scroll.backgroundColor = PDPhotoBrowserBackgrounColor;
            _scroll = scroll;
...
{
    PDWaitingView *waiting = [[PDWaitingView alloc] init];
    waiting.bounds = CGRectMake(0, 0, 100, 100);
    waiting.mode = PDWaitingViewProgressMode;
    _waitingView = waiting;
    [self addSubview:waiting];
    
    
    __weak PDBrowserImageView *imageViewWeak = self;
...
//    NSLog(@"%@",recognizer);
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded && temp < 1) {
        [self eliminateScale];
    }
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = PDPhotoBrowserBackgrounColor;
